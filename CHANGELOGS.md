...
- Desktop app: 235 tests across 28 files pass after adding crash/error diagnostics (Phase 4.2) and What-if planning mode (Phase 5). The new opt-in error log captures React errors, unhandled promise rejections, and app errors to a bounded localStorage ring buffer (last 50), with an ErrorBoundary wrapping the app, global error handlers wired at startup, and a collapsible error-log section in the Diagnostics panel. The What-if sidebar tab clones the current character into a sandbox, supports gear swaps from the full item database, enchant editing from the verified Classic library, talent tab editing, level changes, and side-by-side DPS/survival score deltas with stat changes, set bonus gains/losses, and scenario save/restore. 45 new tests cover clone isolation, modification, comparison, persistence, and error-log behavior. Full lint, TypeScript/production build, and the addon behavior harness pass locally.
- Completed optimization foundation: whole-loadout cap handling, enchant resolution, deterministic set thresholds, curated deterministic effects, declarative Mage Arcane/Fire/Frost, Shadow Priest, Affliction Warlock, Elemental Shaman, Retribution Paladin, and Protection Paladin school modules, explicit Automatic/Solo/Dungeon-boss/Raid-boss/Custom effect windows, Pareto DPS/survival recommendation lenses, versioned reproducible analysis snapshots, five-lens deterministic sensitivity classification, What-if planning mode, and crash/error diagnostics.
...
Accessibility completion (aria-live regions, screen-reader labels on icon-only buttons) closes Phase 4.4. Offline-first icon/data cache status is the next Phase 4 item with no external dependency.
